		...
		try {
			...
			server.addEventListener("createGroup", GroupPayload.class, new DataListener<GroupPayload>() {
				...
				public void onData(SocketIOClient client, GroupPayload data, AckRequest ackRequest) {
					...
					if (data != null) {
						...
						String tid = groupChatService.createGroup("name"+counter++, data.getParticipantsUids(),
					...
					if(data.getParticipants()!=null){
						for (String uid : data.getParticipants()) {
							if(uid==null){uid=userNameMap.get(client);};
						...
						ProducerRecord<String, String> producerRecord = new ProducerRecord<String, String>(userNameMap.get(client),
								json.toJSONString());
						kafkaProducer.send(producerRecord);
					}
			...
			server.addEventListener("addParticipant", GroupPayload.class, new DataListener<GroupPayload>() {
				...
				public void onData(SocketIOClient client, GroupPayload data, AckRequest ackRequest) {
					...
					for (String uid : groupChatService.getParticipants(data.getTid())) {
						ProducerRecord<String, String> producerRecord = new ProducerRecord<String, String>(uid,
								json.toJSONString());
						kafkaProducer.send(producerRecord);
					}
			...
			server.addEventListener("removeParticipantFromGroup", GroupPayload.class, new DataListener<GroupPayload>() {
				...
				public void onData(SocketIOClient client, GroupPayload data, AckRequest ackRequest) {
					...
					if (data != null) {
						...
						thread.setTs(new Date());
						thread.setMsg(data.getMessage());
						groupChatService.removeParticipant(data.getParticipant(), data.getTid());
						groupChatService.appendChatMessageToGroupThread(thread);
					}
			...
			server.addEventListener("removeAdminFromGroup", GroupPayload.class, new DataListener<GroupPayload>() {
				...
				public void onData(SocketIOClient client, GroupPayload data, AckRequest ackRequest) {
					...
					if (data != null) {
						...
						thread.setTs(new Date());
						thread.setMsg(data.getMessage());
						groupChatService.removeAdmin(data.getParticipant(), data.getTid());
						groupChatService.appendChatMessageToGroupThread(thread);
					}
